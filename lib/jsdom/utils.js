...
if (!canvasLibrary) {
  try {
    ...
    canvasLibrary = "skia-canvas";
    canvasInstalled = true;
  } catch (e) {
    // skia-canvas not installed
  }
}
if (canvasInstalled) {
  const Canvas = require(canvasLibrary);
  if (canvasLibrary === "canvas" && typeof Canvas.createCanvas === "function") {
    // In browserify, the require will succeed but return an empty object
    exports.Canvas = Canvas;
  } else if (canvasLibrary === "skia-canvas") {
    console.log('[JSDOM TEMP DEBUG] Registering skia-canvas'); // eslint-disable-line
    Canvas.createCanvas = (...args) => new Canvas.Canvas(...args);
    exports.Canvas = Canvas;
  }
}
